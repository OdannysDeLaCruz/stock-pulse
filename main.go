...
//     // Relaciones
//     StockPriceHistory []StockPriceHistory `gorm:"foreignKey:StockID;constraint:OnDelete:CASCADE" json:"price_history"`
// }
...
// 			"new_item_price_history": NewItemStockPriceHistory {
//                 TargetFrom:  roundToTwoDecimals(stock.NewItemStockPriceHistory.TargetFrom),
//                 TargetTo:    roundToTwoDecimals(stock.NewItemStockPriceHistory.TargetTo),
...
//         if err := DB.Create(&StockPriceHistory).Error; err != nil {
...
//         DB.Where("ticker = ?", stock.Ticker).Order("time DESC").Limit(10).Find(&StockPriceHistory)

//         slices.Reverse(StockPriceHistory)

//         // Formatear datos para la gráfica
//         var StockPriceHistoryMapped = make([]map[string]interface{}, 0)
...
//     var StockPriceHistory []StockPriceHistory
//     result = DB.Where("ticker = ?", stock.Ticker).Order("time ASC").Limit(10).Find(&StockPriceHistory)
...
// 	for _, stock := range stocks {
// 		var StockPriceHistory []StockPriceHistory
// 		DB.Where("ticker = ?", stock.Ticker).Order("time ASC").Limit(10).Find(&StockPriceHistory)
